...
function runSignalEvaluation(fireSignals = true) {
  ...
  const tlL = (typeof scoreTrendline === 'function') ? scoreTrendline('L', ST.candles, ind.atr) : { score: 0 };
  const tlS = (typeof scoreTrendline === 'function') ? scoreTrendline('S', ST.candles, ind.atr) : { score: 0 };
  const volL = (typeof scoreVolume === 'function') ? scoreVolume('L', ST.candles) : { score: 0 };
  const volS = (typeof scoreVolume === 'function') ? scoreVolume('S', ST.candles) : { score: 0 };
  const atrD1 = (ind.atrD1 || ind.atr * 12) || ind.atr;
  const gapL = (typeof scoreGap === 'function') ? scoreGap('L', ST.candles, atrD1) : { score: 0 };
  const gapS = (typeof scoreGap === 'function') ? scoreGap('S', ST.candles, atrD1) : { score: 0 };
  const mtvL = (typeof scoreMTV === 'function') ? scoreMTV('L', ST.candles, ind.atr) : { score: 0 };
  const mtvS = (typeof scoreMTV === 'function') ? scoreMTV('S', ST.candles, ind.atr) : { score: 0 };
...
function onM5CandleClose(candle) {
  ...
  // Fire via telegram (lightweight format) if function exists
  if (typeof sendTelegramM5 === 'function') {
    sendTelegramM5({
      side: m5Result.side, entry, sl, tp1, tp2, tp3,
      score: m5Result.score, triggers: m5Result.triggers, riskPct,
    });
  } else if (typeof sendTelegram === 'function') {
    const msg = `⚡ M5 ${isLong ? 'BUY' : 'SELL'} · XAUUSD\n` +
      `Score: ${m5Result.score}/100\n` +
      `Entry: ${entry.toFixed(2)} · SL: ${sl.toFixed(2)} (${slDist.toFixed(2)}$)\n` +
      `TP1: ${tp1.toFixed(2)} (1.5R) · TP2: ${tp2.toFixed(2)} (3R) · TP3: ${tp3.toFixed(2)} (5R)\n` +
      `Triggers: ${m5Result.triggers.join(' + ')}\n` +
      `Risk: ${riskPct.toFixed(2)}% (M5 tight)`;
    sendTelegram(msg);
  }
}
